...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/bank_account_consumer.log"),
...
@stream_engine.stream('postgres.main.bank_accounts')
async def main(stream: Stream):
    async for cr in stream:
        val = json.loads(cr.value.decode('utf-8'))
        data = {
            'id' : val['payload']['after']['id'],
            'bank_id' : val['payload']['after']['bank_id'],
            'user_id' : val['payload']['after']['user_id'],
            'currency_code' : val['payload']['after']['currency_code'],
            'account' : val['payload']['after']['account'],
            'balance' : val['payload']['after']['balance'],
            'status' : val['payload']['after']['status'],
            'update_dt' : datetime.strptime(
                val['payload']['after']['update_dt'],
                '%Y-%m-%dT%H:%M:%S.%fZ'
            ).replace(tzinfo=timezone.utc),
            'registration_dt' : datetime.strptime(
                val['payload']['after']['registration_dt'],
                '%Y-%m-%dT%H:%M:%S.%fZ'
            ).replace(tzinfo=timezone.utc)
        }
