...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("logs/user_consumer.log"),
...
@stream_engine.stream('postgres.main.users')
async def main(stream: Stream):
    async for cr in stream:
        val = json.loads(cr.value.decode('utf-8'))
        data = {
            "id": val['payload']['after']['id'],
            "user_name": val['payload']['after']['user_name'],
            "user_surname": val['payload']['after']['user_surname'],
